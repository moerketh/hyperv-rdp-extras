...
| Output identity | The crate's DEFAULT is the neutral `rdp` (kscreen `Virtual-rdp`), via `VirtualOutputConfig`'s `Default` impl; the fork passes its own name (`lamco`) explicitly at both the manager (`with_config`) and the guard (`engage_with`). NOTE: this replaced the crate's original `fork_default()` constructor — a deliberate breaking API change at 0.1.0 (never published), accepted over the clippy-flagged alternative of an ambiguous inherent `default()` method. The `Virtual-lamco` strings in the parser tests are arbitrary exclusion-parameter fixtures (real-world Hyper-V sample data for the parameterized, exact-match exclusion), not defaults |
